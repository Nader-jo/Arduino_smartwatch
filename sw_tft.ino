            ...
            while (is_menu == 6)
            {
              ...
              if ((select() == 1) && (UtililyMenuPosition == 2))
              {
                ...
                while (is_menu == 7)
                {
                  ...
                  if (TorchMenuPosition == 1)
                    LED_status = (LED_status + select()) % 2;
...
uint8_t watchFace(uint8_t screen, uint8_t prev, uint8_t LastPrev)
{
  ...
  if (screen == 0)
  {
    watch(alarm_status, message_status, call_status, bluetooth_status);
    prev = screen;
  }
  else if (screen == 1)
  {
    numWatch(alarm_status, message_status, call_status, bluetooth_status);
    prev = screen;
  }
  return (prev);
}
